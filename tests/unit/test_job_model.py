...
from tests.fixtures import invalid_type_job_one
...
class TestJobModel(unittest.TestCase):
    def test_when_job_is_initialized_required_params_are_provided(self):
        job_one = Job(amount=-18, user_id=fake_user_id, fee_type='Tax')
    ...
    def test_when_job_is_created_and_amount_is_not_given_then_amount_not_present_exception_is_raised(self):
        assert_that(Job).raises(AmountNotPresentException).when_called_with(user_id=fake_user_id, fee_type='ServiceFee')

    def test_when_job_is_created_and_userid_is_not_given_then_userid_not_present_exception_is_raised(self):
        assert_that(Job).raises(UserIDNotPresentException).when_called_with(amount=20, fee_type='ServiceFee')
    ...
    def test_when_job_is_created_and_invalid_type_is_given_then_typeerror_is_raised(self):
        assert_that(Job).raises(TypeError).when_called_with(**invalid_type_job_one).is_equal_to("Fee Type is invalid")
